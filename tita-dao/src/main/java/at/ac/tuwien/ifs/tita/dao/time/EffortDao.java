...
public class EffortDao extends GenericHibernateDao<Effort, Long> implements IEffortDao {
    ...
    public List<Effort> getTimeEffortsMonthlyView(Integer year, Integer month, TiTAUser user) {
        ...
        return findByCriteriaOrdered(new Criterion[] { Restrictions.between("date", start.getTime(), end.getTime()),
                Restrictions.eq("deleted", false), Restrictions.eq("user", user) }, new Order[] { Property.forName(
                "date").asc() }, new String[] {});
    }
    ...
    public List<Integer> getTimeEffortsYears(Long userId) {
        String query = "select distinct year(te.date) from Effort te where deleted=false " + " and te.user = " + userId
                + " order by 1 desc";
        Query q = getSession().createQuery(query);
        return q.list();
    }
