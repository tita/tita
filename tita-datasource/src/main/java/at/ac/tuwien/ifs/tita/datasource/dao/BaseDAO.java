 ...
 *        BaseEntity
 * 
 *        BaseDAO is a fully functionally Class for persisting any sub-types of
 *        the BaseEntity Object. If additional functionality is needed simply
 *        extend this class and implement the needed Methods.
 * 
 */
@Repository
public class BaseDAO<DomainClass extends BaseEntity> implements IBaseDAO<DomainClass> {
     ...
     * 
     * @param entity - A sub-type of BaseEntity
     */
    public DomainClass save(DomainClass entity) throws TitaDAOException {
        ...
        try {
            if (entity.getId() == null) {
                ...
                log.debug("Entity with Id: " + entity.getId() + " successfuly  saved!");
                return retDomain;
            } else {
                log.debug("Entity seems to exist, trying to update");
                DomainClass retDomain = entityManager.merge(entity);
                log.debug("Entity with Id: " + retDomain.getId() + " successfuly updated!");
                return entity;
            }
        } catch (Exception e) {
            log.debug("Exception catched while trying to save Entity with Id: " + entity.getId());
    ...
    public void update(DomainClass entity) throws TitaDAOException {
        if (entity == null || entity.getId() == null) {
            log.debug("Entity is null, Exception will be thrown");
            throw new TitaDAOException("Entity cannot be updated if its ID or itself is null");
        }

        try {
            log.debug("Trying to update Entity with Id: " + entity.getId());
            entityManager.merge(entity);
            log.debug("Entity with Id: " + entity.getId() + " successfuly updated!");
        } catch (Exception e) {
            log.debug("Exception catched while trying to update Entity with Id: " + entity.getId());
            throw new TitaDAOException("Exception was thrown: \n" + "Cause: " + e.getCause() + "\n" + "Error: "
                    + e.getMessage());
        }
    }
    ...
    public void delete(DomainClass entity) throws TitaDAOException {
        if (entity == null || entity.getId() == null) {
            ...
            throw new TitaDAOException("Entity cannot be deleted if its ID or itself is null");
        }

        try {
            log.debug("trying to remove Entity with Id: " + entity.getId());
            entityManager.remove(entityManager.merge(entity));
            log.debug("Entity with Id: " + entity.getId() + " successfuly removed");
        } catch (Exception e) {
            log.debug("Exception catched while trying to delete Entity with Id: " + entity.getId());
            throw new TitaDAOException("Exception was thrown: \n" + "Cause: " + e.getCause() + "\n" + "Error: "
                    + e.getMessage());
        }
    }
     ...
     * 
     * @param the id of the existing object
     * @return the object
     */
    public DomainClass getById(Class<? extends DomainClass> persistentClass, Long id) throws TitaDAOException {
        ...
        try {
            ...
            log.debug("Exception catched while trying to get Entity with Id: " + id);
            throw new TitaDAOException("Exception was thrown: \n" + "Cause: " + e.getCause() + "\n" + "Error: "
                    + e.getMessage());
        }

        if (entity == null)
            throw new TitaDAOException("Entity with id: " + id + " cannot be found");
     ...
     * 
     * @param the criteria for searching Domain objects
     */
    @SuppressWarnings("unchecked")
    public List<DomainClass> search(IBaseCriteria criteria) throws TitaDAOException {
        if (criteria == null) {
            log.debug("Criteria is null, Exception will be thrown");
            throw new TitaDAOException("Entities cannot be search, because criteria is null");
        }
    ...
    public IBaseCriteria<DomainClass> createCriteria(DomainClass domain) throws TitaDAOException {
