...
public class TimedTaskCoordinatorTest extends MantisBaseTest {
    ...
    private IssueTrackerMantisDao mantisDao = new IssueTrackerMantisDao();
    ...
    public void messureTimeForOneTaskShouldSucceed(){
        ...
        assertEquals(new Integer(4),
                     TiTATimeConverter.getSeconds(((ITimedTask)task).getDuration()));
        //CHECKSTYLE:ON
        
    }
    
    /**
     * Messures time for more tasks parallel and return messured time of 2 sec. for each.
     */
    @Test
    public void messureTimeForMoreTaskShouldSucceed(){
        ...
        assertEquals(new Integer(7),TiTATimeConverter.getSeconds(
                       tiCo.getTaskDurations().get(((ITimedTask)(tasks.get(0))).getTimedTaskId())));
        assertEquals(new Integer(3),TiTATimeConverter.getSeconds(
                       tiCo.getTaskDurations().get(((ITimedTask)(tasks.get(1))).getTimedTaskId())));
        assertEquals(new Integer(1),TiTATimeConverter.getSeconds(
                       tiCo.getTaskDurations().get(((ITimedTask)(tasks.get(2))).getTimedTaskId())));
        //CHECKSTYLE:ON
    }
}
